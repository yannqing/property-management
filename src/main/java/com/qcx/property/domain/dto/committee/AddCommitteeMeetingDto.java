...
/**
 * @description: 添加委员会会议DTO
 * @author: yannqing
 * @create: 2025-04-16 12:10
 * @from: <更多资料：yannqing.com>
 **/
@Data
@Schema(name = "AddCommitteeMeetingDto", description = "添加委员会会议请求参数")
public class AddCommitteeMeetingDto implements Serializable {
    /**
     * 委员会ID
     */
     ...
     * 会议日期时间
     */
    @Schema(description = "会议日期时间(格式：yyyy-MM-dd或yyyy-MM-dd HH:mm:ss)", requiredMode = Schema.RequiredMode.REQUIRED)
     ...
     * 状态(0筹备中,1已召开,2已取消)
     */
    @Schema(description = "状态(0筹备中,1已召开,2已取消)", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    /**
     * 将DTO转换为实体
     * @param addCommitteeMeetingDto DTO对象
     * @return 实体对象
     */
    public static CommitteeMeeting toEntity(AddCommitteeMeetingDto addCommitteeMeetingDto) {
        if (addCommitteeMeetingDto == null) {
            return null;
        }
        CommitteeMeeting committeeMeeting = new CommitteeMeeting();
        BeanUtils.copyProperties(addCommitteeMeetingDto, committeeMeeting);
